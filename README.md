...
* Implémentation du système en **langage IL (Instruction List / AWL)**, optimisé pour les temps de cycle courts de l'automate.

### 3. Partie 1 : Système de Remplissage de Cuve
* **GRAFCET & LADDER Interactifs :** Observez le flux de puissance électrique (Power Flow) illuminer les contacts et les bobines en temps réel.
* **Explication du mécanisme :** Analyse textuelle de la condition de niveau bas (LSL), du circuit d'auto-maintien de la pompe, et de la validation de stabilité (Temporisateur TON de 10s) déclenchée par le niveau haut (LSH).
...
* **Explication du mécanisme :** Description de la séquence asynchrone (Départ cycle, Serrage), de l'activation parallèle des usinages (Fraisage et Lamage), et de la contrainte matérielle critique liée au distributeur **monostable** lors de l'éjection.
...
Ce projet est conçu pour être une documentation interactive **100% autonome**. Il ne nécessite aucune installation de serveur (ni Node.js, ni XAMPP), ni de base de données, ni de fichiers externes (images).
